...
contract XVS is Tokenlock {
    ...
    uint256 public constant totalSupply = 30000000e18; // 30 million XVS
        ...
        returns (bool)
    {
        uint96 amount = safe96(
            rawAmount,
            "XVS::transfer: amount exceeds 96 bits"
        );
    ...
    function transferFrom(
        ...
        uint96 amount = safe96(
            rawAmount,
            "XVS::approve: amount exceeds 96 bits"
        );
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "XVS::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "XVS::delegateBySig: invalid nonce"
        );
        ...
        returns (uint96)
    {
        require(
            ...
            "XVS::getPriorVotes: not yet determined"
        );
    ...
    function _transferTokens(
        ...
        require(
            src != address(0),
            "XVS::_transferTokens: cannot transfer from the zero address"
        );
        require(
            dst != address(0),
            "XVS::_transferTokens: cannot transfer to the zero address"
        );

        balances[src] = sub96(
            balances[src],
            amount,
            "XVS::_transferTokens: transfer amount exceeds balance"
        );
        balances[dst] = add96(
            balances[dst],
            amount,
            "XVS::_transferTokens: transfer amount overflows"
        );
        ...
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(
                    srcRepOld,
                    amount,
                    "XVS::_moveVotes: vote amount underflows"
                );
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(
                    dstRepOld,
                    amount,
                    "XVS::_moveVotes: vote amount overflows"
                );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "XVS::_writeCheckpoint: block number exceeds 32 bits"
        );
